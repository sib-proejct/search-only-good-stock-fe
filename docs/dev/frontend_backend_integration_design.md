...
- [x] FE와 BE 개발 Origin이 일치하고 CORS 오류가 없다.
- [x] FE DTO가 최신 OpenAPI의 camelCase와 nullable 계약을 반영한다.
- [x] 목록 API의 페이지 객체와 모든 지원 쿼리를 사용한다.
- [x] 상세 API의 규칙, 자본행동, DCF, 사유와 경고를 표시한다.
- [x] 주식 화면에서 자동 Mock fallback과 `MOCK_STOCKS` 직접 참조가 없다.
- [x] BE에 없는 값을 0, 점수, Mock 또는 추정값으로 표시하지 않는다.
- [x] 로딩, 빈 결과, 404, 422, 네트워크·서버 오류를 구분한다.
- [x] BE와 FE 품질 명령이 모두 성공한다.
- [x] 필수 브라우저 통합 시나리오가 모바일·데스크톱에서 통과한다.
