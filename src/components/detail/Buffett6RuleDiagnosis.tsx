  ...
  return (
    <div className="bg-white dark:bg-[#1C1C1E] rounded-3xl p-7 border border-black/[0.06] dark:border-white/[0.08] shadow-sm flex flex-col justify-between space-y-5 transition-colors duration-300">
      ...
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs text-[#86868B] font-medium bg-[#F5F5F7] dark:bg-[#2C2C2E] px-3 py-1 rounded-full border border-black/[0.03] dark:border-white/[0.06]">
          {t('updatedToday')}
        </span>
      </div>

      {/* 6 Bento Tiles Grid (3 cols x 2 rows) */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3.5">
        {rules.map((rule, idx) => {
          ...
          return (
            <div
              key={idx}
              className="bg-white dark:bg-[#252528] border border-black/[0.06] dark:border-white/[0.06] rounded-2xl p-4 flex flex-col justify-between hover:border-black/[0.12] dark:hover:border-white/[0.15] transition-all"
            >
              ...
              <div>
                ...
                <span className="text-xs font-bold text-[#1D1D1F] dark:text-[#F5F5F7] mt-0.5 block tracking-tight">
                  {rule.value}
                </span>
              </div>
            </div>
