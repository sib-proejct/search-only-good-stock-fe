...
export const MarketBenchmarkCard: React.FC<MarketBenchmarkCardProps> = ({
  ...
  const dilutedSharesNum =
    currentMarket.dilutedShares !== null && currentMarket.dilutedShares !== undefined
      ? Number(currentMarket.dilutedShares)
      : null;
  const formattedDilutedShares =
    dilutedSharesNum === null || isNaN(dilutedSharesNum)
      ? '—'
  ...
  const riskFreeRateNum =
    currentMarket.riskFreeRate !== null && currentMarket.riskFreeRate !== undefined
      ...
      <div className="flex items-center justify-between pb-3 border-b border-black/[0.04] dark:border-white/[0.06]">
        <div>
          ...
          <p className="text-xs text-[#86868B] mt-0.5 font-normal">
            {isKo
              ? '현재 시장 가격, 국채 무위험 수익률 및 1달러 유보이익 테스트 비교 지표'
        ...
        {currentMarket.asOf && (
          <div className="text-[11px] font-mono text-[#86868B] flex items-center gap-1">
            ...
            <span>{isKo ? `${currentMarket.asOf.slice(0, 10)} 기준` : `As of ${currentMarket.asOf.slice(0, 10)}`}</span>
          </div>
        )}
      </div>
        ...
        <div className="p-4 rounded-2xl bg-[#F5F5F7]/80 dark:bg-[#252528]/60 border border-black/[0.04] dark:border-white/[0.06] space-y-2">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            {benchmarkPoints.map((bp, idx) => {
              const idxNum =
                bp.indexValue !== null && bp.indexValue !== undefined
                  ? Number(bp.indexValue)
                  : null;
              return (
                <div
                  key={idx}
            ...
            <div className="mt-3 overflow-x-auto max-h-60 overflow-y-auto border border-black/[0.04] dark:border-white/[0.06] rounded-2xl">
              <table className="w-full text-left text-xs">
                <thead className="sticky top-0 bg-[#F5F5F7] dark:bg-[#252528] text-[#86868B] font-semibold border-b border-black/[0.04] dark:border-white/[0.06]">
                  <tr>
                    <th className="py-2.5 px-3">{isKo ? '회계분기' : 'Quarter'}</th>
                    <th className="py-2.5 px-3">{isKo ? '기준일' : 'Date'}</th>
                    <th className="py-2.5 px-3 text-right">{isKo ? '수정 종가' : 'Adjusted Close'}</th>
                    <th className="py-2.5 px-3 text-right">{isKo ? '주당순자산 (BVPS)' : 'BVPS'}</th>
                    <th className="py-2.5 px-3 text-right">{isKo ? 'P/B 비율' : 'P/B Ratio'}</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-black/[0.03] dark:divide-white/[0.04] font-mono tabular-nums">
                  {quarterlyBookPrices.map((q, idx) => {
                    ...
                    const pb =
                      !isNaN(adjCloseNum) && !isNaN(bvpsNum) && bvpsNum > 0
                        ? (adjCloseNum / bvpsNum).toFixed(2)
                        : '—';
                    return (
                      <tr key={idx} className="hover:bg-[#F9FAFB] dark:hover:bg-[#252528]/50">
                        <td className="py-2 px-3 font-semibold">
                          {q.fiscalYear} Q{q.fiscalQuarter}
                        </td>
                        <td className="py-2 px-3 text-[#86868B]">{q.periodEnd.slice(0, 10)}</td>
                        <td className="py-2 px-3 text-right">
                          {formatPrice(q.adjustedClosePrice)}
                        </td>
                        <td className="py-2 px-3 text-right font-bold text-[#0071E3] dark:text-[#2997FF]">
                          {formatPrice(q.bvps)}
                        </td>
                        <td className="py-2 px-3 text-right text-[#86868B]">
                          {pb}
                          {pb !== '—' ? 'x' : ''}
                        </td>
                      </tr>
